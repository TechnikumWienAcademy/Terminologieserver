...
public class Mail
{
  ...
  {
    return DBSysParamAdmin.instance().getStringValue("weblink", null, null);
  }
  
  private static String getAppLinkCollaboration()
  {
    return DBSysParamCollab.instance().getStringValue("weblink", null, null);
  }
  
  private static String getTermBrowserLink(){
      return DBSysParamAdmin.instance().getStringValue("termbrowserlink", null, null);
  }
  ...
  public static String sendNewPassword(String Benutzername, String Passwort, String BenutzerEmail)
  {
    try
    {
      if (logger.isDebugEnabled())
      {
        ...
        logger.debug(BenutzerEmail);
        logger.debug(Benutzername);
      }
      ...
      adr[0]= BenutzerEmail;
      return smtp.sendMail(adr, betreff, M_AUT.MAIL_START + text + M_AUT.getInstance().getMailFooter());
    }
    catch (Exception e)
    {
