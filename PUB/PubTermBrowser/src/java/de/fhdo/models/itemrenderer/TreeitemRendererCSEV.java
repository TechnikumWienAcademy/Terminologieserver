...
//  protected TreeNode tn;
            ...
            switch (parentWindow.getAssociationMode())
            {
              case 1: // Crossmapping ////////////////////////////                                                                                                                                                                                                                             
                ...
                if (csev_source != null && csev_target != null)
                {
                  ...
                  // Login                            
                  de.fhdo.terminologie.ws.conceptassociation.LoginType login = new de.fhdo.terminologie.ws.conceptassociation.LoginType();
  ...
  protected void renderCSEVDisplay(Treerow dataRow, Object data, Treeitem treeItem, TreeNode treeNode)
  {
    ...
    if(contentMode == ContentConcepts.CONTENTMODE_VALUESET){
        ValueSetVersion vsv = (ValueSetVersion)((ContentConcepts) parentWindow).getSource();
				...
				if (vsv.getValueSet().getName().contains("ELGA_Laborparameter")){
    ...
    if (csev.getCodeSystemConcepts().size() > 0)
    {
      ...
      if (parentWindow.getContentMode() == ContentConcepts.CONTENTMODE_VALUESET)
      {
        ...
        catch (ComponentNotFoundException e){e.printStackTrace();}
      }
      ...
      { // TODO: UserAccount.getShowPrefferedLanguage()
        ...
        while (it.hasNext())
        {
          ...
          if (csct.getLanguageId() != null && csct.getLanguageId() == 33)
          { // User.getLanguageId();
            ...
            break;
				...
				//if no translation is available use regular term
				if (lName.getValue().equals("")){
					lName.setValue(csc.getTerm());
				}
  ...
  protected void renderCSEVContextMenu(Menupopup contextMenu, final Treeitem ti, Treerow dataRow, final Object data)
  {
    ...
    Menuitem miRemoveVS = null;// = new Menuitem(Labels.getLabel("treeitemRendererCSEV.miRemoveFromVS"));
		Menuitem miDetailsCS = new Menuitem(Labels.getLabel("treeitemRendererCSEV.miDetailsParentCS"));
...
//                tn2.showDeepLinkInMessagebox("","",new ArrayList<String>());
    ...
    miRemoveVS.addEventListener(Events.ON_CLICK, new EventListener()
    {
      public void onEvent(Event event) throws Exception
      {
		...
		miDetailsCS.addEventListener(Events.ON_CLICK, new EventListener() {

			public void onEvent(Event t) throws Exception {
				((ContentConcepts) parentWindow).showPopupConcept(PopupWindow.EDITMODE_DETAILSONLY, true);
			}
		});
    ...
    miDeepLink.setParent(contextMenu);
		...
		if (contentMode == ContentConcepts.CONTENTMODE_VALUESET){
			miDetailsCS.setParent(contextMenu);
			miDetails.setParent(contextMenu);
		}
		
		//Matthias Änderung der Routine für das Kontextmenü "zu Valueset hinzufügen"
    ...
    if (SessionHelper.isCollaborationActive())
    {
      ...
      if (contentMode == ContentConcepts.CONTENTMODE_CODESYSTEM)
      {
        if(AssignTermHelper.isAnyUserAssigned(((CodeSystemVersion)source).getCodeSystem().getId(), "CodeSystem")){//Kein IV, kein Vorschlag
      ...
      if (contentMode == ContentConcepts.CONTENTMODE_VALUESET)
      {
            if(AssignTermHelper.isAnyUserAssigned(((ValueSetVersion)source).getValueSet().getId(), "ValueSet")){//Kein IV, kein Vorschlag
                new Menuseparator().setParent(contextMenu);
  ...
  public void render(Treeitem ti, Object treeNode, int index) throws Exception
  {
    ...
    else
    {
      ...
      else logger.debug("Object-Type nicht gefunden (data instanceof " + "" + ")");
    }
  }
