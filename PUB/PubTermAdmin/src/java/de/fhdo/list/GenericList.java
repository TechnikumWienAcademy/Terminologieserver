...
public class GenericList extends Window implements IDoubleClick, IUpdateData
{
  ...
  private Map<Integer, GenericList.FilterType> filterMap;
  ...
  public GenericList()
  {
    ...
    filterMap = new HashMap<Integer, GenericList.FilterType>();
  }
  ...
  private void filterChanged(GenericListHeaderType head, Date Begin, Date End)
  {
    ...
    GenericList.FilterType filterType;
    ...
    else
    {
      if (Begin != null || End != null)
      {
        filterType = new GenericList.FilterType();
  ...
  private void executeFilter()
  {
    ...
    if (listModel != null)
    {
      for (int i = 0; i < listModel.getSize(); ++i)
      {
        ...
        for (GenericList.FilterType ft : filterMap.values())  // Alle Filter durchgehen
        {
      ...
      for (int i = 0; i < listHeader.size(); ++i)
      {
        ...
        if (head.isShowFilter())
        {
          ...
          if (head.getDatatype() instanceof String && head.getDatatype().toString().equalsIgnoreCase("String"))
          {
            ...
            //tbFilter.setStyle("");
            /*tbFilter.addEventListener("ON_MOUSE_UP", new EventListener<Event>()
            {
              public void onEvent(Event event) throws Exception
              {
