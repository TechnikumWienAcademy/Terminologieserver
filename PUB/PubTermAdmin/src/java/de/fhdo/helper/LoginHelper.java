...
import de.fhdo.terminologie.ws.authorizationPub.LoginResponse;
...
public class LoginHelper
{
  ...
  public String login(String username, String password)
  {
    ...
    try
    {
      ...
      if (userList.size() == 1)
      {
        ...
        if (response.getReturnInfos().getStatus() == Status.OK)
        {
          ...
          {
            //Anmelden auf Pub
            de.fhdo.terminologie.ws.authorizationPub.LoginRequestType req = new de.fhdo.terminologie.ws.authorizationPub.LoginRequestType();
            req.setLogin(new de.fhdo.terminologie.ws.authorizationPub.LoginType());
            req.getLogin().setUsername(username);
            req.getLogin().setPassword(passwordSalted);

            LoginResponse.Return resp = login_pub(req);
						System.out.println("Anmeldung an Public: " + resp.getReturnInfos().getMessage());
            if(resp.getReturnInfos().getStatus().equals(de.fhdo.terminologie.ws.authorizationPub.Status.OK))
            {
              finalPubSessionId = resp.getLogin().getSessionID();
              session.setAttribute("pub_session_id", resp.getLogin().getSessionID());
            }
          }
        }
        
        logger.debug("session_id: " + session.getAttribute("session_id"));
        logger.debug("pub_session_id: " + session.getAttribute("pub_session_id"));
      }
    ...
    returnValue += cRole + ";" + cId+";"+finalPubSessionId+";";
