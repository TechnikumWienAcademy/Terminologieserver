...
//        ((Borderlayout)getRoot()).getCenter().setTitle(Labels.getLabel("common.mainView"));                
    ...
    private void checkForTermId()
    {
        String termIdString = Executions.getCurrent().getParameter("termId");
        String contentConceptsString = Executions.getCurrent().getParameter("contentConcepts");

        long contentConcepts = 0;
        if (contentConceptsString != null && !contentConceptsString.equals(""))
        {
            contentConcepts = Long.parseLong(contentConceptsString);
        }

        if (termIdString != null && !termIdString.equals(""))
        {
            long termId = Long.parseLong(termIdString);

            if (contentConcepts == ContentConcepts.CONTENTMODE_VALUESET)
            {

                TreeNode root = (TreeNode) treeVS.getModel().getRoot();

                List<TreeNode> tnListVS = root.getChildren();

                for (TreeNode vs : tnListVS)
                {
                    List<TreeNode> tnListVSV = vs.getChildren();

                    for (TreeNode csv : tnListVSV)
                    {
                        ValueSetVersion valueSetVersion = (ValueSetVersion) csv.getData();
                        if (valueSetVersion.getVersionId() == termId)
                        {
                            treeActive = treeVS;
                            int[] path = treeActive.getModel().getPath(vs);
                            Treeitem ti = treeActive.renderItemByPath(path);
                            selectedVS = (ValueSet) vs.getData();
                            selectedVSV = valueSetVersion;
                            ti.setValue(vs);
            ...
            if (contentConcepts == ContentConcepts.CONTENTMODE_CODESYSTEM)
            {

                TreeNode root = (TreeNode) treeCS.getModel().getRoot();

                for (Object node : root.getChildren())
                {
                    TreeNode n = (TreeNode) node;
                    List<TreeNode> tnListCS = n.getChildren();

                    for (TreeNode cs : tnListCS)
                    {
                        List<TreeNode> tnListCSV = cs.getChildren();

                        for (TreeNode csv : tnListCSV)
                        {
                            CodeSystemVersion codeSystemVersion = (CodeSystemVersion) csv.getData();
                            if (codeSystemVersion.getVersionId() == termId)
                            {
                                treeActive = treeCS;
                                int[] path = treeActive.getModel().getPath(n);
                                Treeitem ti = treeActive.renderItemByPath(path);
                                selectedCS = (CodeSystem) cs.getData();
                                selectedCSV = codeSystemVersion;
                                ti.setValue(n);
                                treeActive.setSelectedItem(ti);
                                loadConceptsBySelectedItem(false, false);
                                return;
...
// Deep Links //////////////////////////////////////////////////////////////////    
    ...
    public void loadConceptsBySelectedItem(boolean draggable, boolean droppable)
    {
        ...
        //Check if handed treeitem is type of domainValue --> method has been called
        //from checkForTermId.
        if (source instanceof DomainValue)
        {
            source = selectedCSV;
        }
...
//      validityRange = selectedVSV.getValidityRange();
    ...
    private void clearSelectedItemsAndButtons()
    {
        selectedCS = null;
        selectedCSV = null;
        selectedVS = null;
