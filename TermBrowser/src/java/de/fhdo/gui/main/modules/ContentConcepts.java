...
//  private LinkedList<TreeNode> openPaths = new LinkedList<TreeNode>();
    ...
    private boolean isContentModeResetNeeded = false;
    private boolean shouldParentCodeBeDisplayed = false;
    ...
    //Method is called when the parent-code-details should be opened for concept in valueset
    public void showPopupConcept(int mode, boolean shouldParentCodeBeDisplayed)
    {
        this.shouldParentCodeBeDisplayed = shouldParentCodeBeDisplayed;
        if (shouldParentCodeBeDisplayed)
        {
            contentMode = CONTENTMODE_CODESYSTEM;
            isContentModeResetNeeded = true;
        }
    ...
    public void showPopupConcept(int editMode, int hierarchyMode)
    {
        ...
        //Matthias: hand valueset name in case of Laborparameter
        if (source instanceof ValueSetVersion)
        {
            String valueSetName = ((ValueSetVersion) source).getValueSet().getName();
            if (valueSetName.contains("ELGA_Laborparameter") && !shouldParentCodeBeDisplayed)
            {
                data.put("Is_ELGA_Laborparameter", true);
            }
        }

        if (editMode == PopupWindow.EDITMODE_CREATE)
        {
            data.put("Association", hierarchyMode);
            /* mode 1 = gleiche ebene, 2 = subebene, 3 = oberste ebene */
        ...
        if (isContentModeResetNeeded)
        {
            contentMode = CONTENTMODE_VALUESET;
            isContentModeResetNeeded = false;
        }

        //Matthias: Reseting
        this.shouldParentCodeBeDisplayed = false;
    }
...
//                loadChildren(tn,4);
    ...
    public void setParent(ContentCSVSDefault parent)
    {
        this.parent = parent;
    }

    public void setTreeConcpts(Tree treeConcpts)
    {
        this.treeConcepts = treeConcpts;
    }
