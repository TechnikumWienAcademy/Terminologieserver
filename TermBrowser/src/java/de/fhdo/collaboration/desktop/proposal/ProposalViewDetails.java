...
import de.fhdo.terminologie.ws.search.LoginType;
...
public class ProposalViewDetails extends Window implements IGenericListActions, IUpdateModal
{
  ...
  private GenericListRowType createObjectRow(Proposalobject po)
  {
    ...
    if (po.getClassname().equalsIgnoreCase("CodeSystemConcept"))
    {
      ...
      if (response.getReturnInfos().getStatus() == Status.OK)
      {
        ...
        {
          int status = response.getCodeSystemEntity().getCodeSystemEntityVersions().get(0).getStatus();
  ...
  public void setProposalView(ProposalView proposalView)
  {
    ...
    public void onNewClicked(String id) {
        ...
        {
          logger.debug("Fehler beim Ã–ffnen der PrivilegeDetails: " + ex.getLocalizedMessage());
          ex.printStackTrace();
        }
    }
    ...
    public void reSaveProposal(){
      ...
      try
      {
        ...
        p.setValidFrom(proposalView.getProposal().getValidFrom());
        p.setValidTo(proposalView.getProposal().getValidTo());
        hb_session.update(p);
        hb_session.getTransaction().commit();
    ...
    public void initProposal(){
        ...
        if(SessionHelper.getCollaborationUserRole().equals(CODES.ROLE_BENUTZER)){
            ((Textbox)getFellow("tb_description")).setDisabled(true);
            ((Datebox)getFellow("db_validFrom")).setDisabled(true);
            ((Datebox)getFellow("db_validTo")).setDisabled(true);
            ((Textbox)getFellow("tb_notiz")).setDisabled(true);
        }
    }
}
