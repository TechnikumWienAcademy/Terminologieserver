...
public class SecurityFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (isLoggedIn(request, response)) {
             ...
             */
						String idp_url = DBSysParam.instance().getStringValue("ipd_url", null, null);
						String weblink = DBSysParam.instance().getStringValue("weblink", null, null);
                                                String[] pureLink = weblink.split("/TermBrowser");
            AuthnRequest ar = AuthnRequestBuilder.buildAuthnRequest(idp_url + "/IDP", pureLink[0] + "/TermAdmin/assertionConsumer", "TermAdmin");
            try {
