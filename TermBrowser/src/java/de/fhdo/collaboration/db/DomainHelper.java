    ...
    if (DomainValueID <= 0)
      return false;
		...
		try {
			...
			if (ueber_dv == null)
			{
				logger.debug("DomainValue mit Ãœber-ID " + UpperID + " existiert nicht");
				return false;
			}
			...
			if (dv != null)
			{
				...
				if (dv.getDomainValuesForDomainValueIdParent()!= null)
				{
					...
					while (it.hasNext())
					{
						...
						{
							logger.debug("Ãœber-Domain bereits gefunden, also nicht hinzufügen");
							gefunden = true;
						}
					}
				}
				...
				if (gefunden == false)
				{
					logger.debug("Ãœber-Domain hinzufügen...");
