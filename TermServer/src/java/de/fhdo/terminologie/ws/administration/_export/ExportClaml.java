...
public class ExportClaml{
            ...
            if (RCSDresponse.getReturnInfos().getStatus() == ReturnType.Status.OK){
                ...
                if (RCSDresponse.getReturnInfos().getStatus() == ReturnType.Status.OK)
                {
                    ...
                    if (codeSystem != null)
                    {
                        ...
                        //Matthias: added incompleteCodesystem and responsibleOrganization
                        ...
                        finally
                        {
                            ...
                            {
                                java.util.logging.Logger.getLogger(ExportClaml.class.getName()).log(Level.SEVERE, null, ex);
                            }
   ...
   // System.out.println(detailsResp.getVocabulary().getDescription());
    ...
    public void createConcepts()
    {
        ...
        conceptsReq.setLogin(this.request.getLogin());
            ...
            try
            {
                ...
                while (itEntitiy.hasNext())
                {
                 ...
                 clazz.getRubric().add(rubric);*/
                                ...
                                if (csev.getStatus() != null)
                                {
                                    createMetadata(METADATA_ATTRIBUTES.status.getCode(), csev.getStatus().toString(), clazz);
                                }
                                if (csev.getStatusDate() != null)
                                {
                                    createMetadata(METADATA_ATTRIBUTES.statusDate.getCode(), csev.getStatusDate().toString(), clazz);
                                }
        ...
        if (value != null && value.length() > 0)
        {
            ...
            if (kind != null)
            {
                if (rubricKinds.containsKey(kind.getCode()) == false)
                {
                    ...
                    rc.setName(checkString(kind.getCode()));
     ...
     logger.info("------------------------");
        ...
        if (metadataList != null)
        {
            for (CodeSystemMetadataValue metadata : metadataList)
            {
                ...
                {
                    //Matthias: check if value is set. If not do not add the metadata
                    if (metadata.getParameterValue() != "")
                    {
                        Meta m = new Meta();
                        m.setName(metadata.getMetadataParameter().getParamName());
                        m.setValue(checkString(metadata.getParameterValue()));
    ...
    private List<clamlBindingXSD.Class> orderConcepts()
    {

        HashMap<String, clamlBindingXSD.Class> clazzMap = new HashMap<String, clamlBindingXSD.Class>();
        List<clamlBindingXSD.Class> sortedList = new LinkedList<clamlBindingXSD.Class>();

        for (clamlBindingXSD.Class clazz : claml.getClazz())
        {
            clazzMap.put(clazz.getCode(), clazz);
        }
        ...
        for (clamlBindingXSD.Class clazz : claml.getClazz())
        {
            ...
            {
                superCode = clazz.getSuperClass().iterator().next().getCode();
            }

            if (superCode.equals(""))
            {
                sortedList.add(clazz);
                clazzMap.remove(code);
            }
            else
            {
                if (clazzMap.get(superCode) != null)
                {
                    sortedList.add(clazzMap.get(superCode));
                    clazzMap.remove(superCode);
                }
        ...
        claml.getClazz().clear();
        for (clamlBindingXSD.Class clazz : sortedList)
        {
            claml.getClazz().add(clazz);
        }
